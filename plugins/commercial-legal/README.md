...
**모든 출력은 변호사 검토 전제의 초안입니다** — 인용된 조문·판례는 법망 MCP로 검증해 표시하고,
...
- **약관규제법 무효·하도급법 적용·B2C 분류는 결론이 아니라 `[검토]` 플래그**입니다. 무효·위반은
  법원·공정위가, 적용 여부는 규모 요건(시행령, 변동)이 결정합니다 — 단정하지 않고 법망 현행 대조로 라우팅합니다.
- **공정위 의결례·표준약관, 한국소비자원·분쟁조정위 결정례는 법망에 포함되어 있지 않습니다.** 인용 시
