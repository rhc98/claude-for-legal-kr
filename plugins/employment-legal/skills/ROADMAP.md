...
| customize | 1:1 | B | CLAUDE.md 부분 수정 |
| matter-workspace | 1:1 | B | 타 플러그인 패턴 그대로 |
...
**A(스캐폴드, 완료) → B(인프라 스킬: cold-start-interview·customize·matter-workspace) →
C(핵심 검토: termination·hiring·worker-classification·wage-hour + labor-committee-procedure) →
D(policy·investigation·leave·industrial-accident + leave-tracker agent) → E(expansion 계열) →
F(E2E 검증).**
