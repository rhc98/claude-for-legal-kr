...
**현재 상태: Phase A·B·C 완료.** 인프라 3스킬 + M&A 핵심 6스킬(신규 merger-control-filing 포함) 포팅됨.
Phase D(이사회·법인) 이후가 남았다. 기준 템플릿은 `../../commercial-legal/skills/`, 상류 원본은
...
## 법망 데이터 갭 (corporate 특유 — `docs/DATA_GAPS.md` 보강 필요)

- **거래소 상장규정**(유가증권시장·코스닥·코넥스) — 한국거래소 자율규정, 법망 미커버 → `[거래소 규정 — 직접 검증 필요]`
- **공정위 기업결합 심사 의결례** — 법망 미커버(기존 갭) → `[공정위 의결 — 직접 검증 필요]`
- **금융위·증선위·금감원 공시/제재 의결** — 부분 커버 → `[금융당국 의결 — 직접 검증 필요]`
- **상업등기 등기선례·등기예규** — 법망 행정규칙 부분 커버, 누락 가능 → 인용 시 검증
- ✓ 잘 커버: 상법·자본시장법·외부감사법·상업등기법·공정거래법 — 법령 + 법원 판례 충실

## 권장 실행 순서

**A(스캐폴드: plugin.json·.mcp.json 법망·CLAUDE.md 모듈형 템플릿·hooks·README·references·marketplace 등록) ✅ →
...
E(상장사 모듈: public-disclosure 등 KR 선도, 후순위) →
