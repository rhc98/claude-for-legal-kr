...
trade-secret-protection) + ip-renewal-watcher 에이전트. 남은 것은 Phase F(E2E 런타임 검증 — 국가법령정보 복구 후).
...
| clearance | 재작성 | C | 상표 등록가능성 — **KIPRIS 검색**(국가법령정보 미커버), 상표법 §34 부등록사유 + 부정경쟁방지법 §2 혼동 overlay |
...
## 국가법령정보 데이터 갭 (ip 특유 — `docs/DATA_GAPS.md` 보강)

- **특허청 KIPRIS**(상표·특허·디자인 등록/출원 원부) — 국가법령정보 미커버 → `[KIPRIS — 직접 검증 필요]`.
  clearance·fto·portfolio의 핵심 갭.
- **특허심판원(IPTAB) 심결** — 국가법령정보 미커버 → `[심판원 심결 — 직접 검증 필요]`
...
**A(스캐폴드: plugin.json·.mcp.json 국가법령정보·CLAUDE.md IP 실무 프로파일·hooks·README·references/ip-currency-watch·
marketplace 등록) ✅ →
...
F(E2E 검증 — 국가법령정보 복구 후: 상표 clearance + 특허 침해 시나리오 1건, 전 인용 verify. 갭 픽스)**
