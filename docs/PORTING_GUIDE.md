...
- `.mcp.json`: 서구 커넥터 제거, **국가법령정보 MCP 추가** (모든 플러그인 공통):
  ```json
  "korean-law": {
    "type": "http",
    "url": "https://mcp.gomdori.app/law?oc=${LAW_OC}",
    "title": "국가법령정보 (korean-law MCP)",
    "description": "한국 법령·판례·헌재결정·행정규칙·자치법규·조약·법령해석례·행정심판재결·위원회결정문..."
  }
  ```
  (`LAW_OC`는 open.law.go.kr에서 발급받는 무료 OC 인증키.)
...
4. **공유 가드레일 섹션 (가장 긴 부분) 한국 재맥락화:**
   ...
   - "Verify user-stated legal facts" — `mcp__korean-law__legal_analysis {"mode": "verify_citations"}` 활용
   - "Source attribution tags" — `[국가법령정보]`, `[법제처]`, `[국회 의안시스템]`, `[PIPC]`, `[KISA]`,
...
2. **본문 한국어화** — 단순 번역 아닌 한국 법체계 재맥락화:
   - 미국·EU 법조 인용 → 한국법 인용 (국가법령정보 호출 코드 스니펫 포함)
...
4. **인용 검증 강제**:
   모든 외부 인용에 출처 태그 의무. `[모델 지식 — 검증 필요]`가 디폴트, 국가법령정보에서
   가져왔으면 `[국가법령정보]`.
...
국가법령정보 등록 → 플러그인 설치 → cold-start-interview 실행 → 핵심 스킬 1-2개에서 실제
한국 법무 시나리오 실행 → 모든 조문 인용 verify 통과 확인.

---

## 국가법령정보 MCP 통합 패턴 (모든 플러그인 공통)

자세한 내용은 [`LAW_MCP_INTEGRATION.md`](LAW_MCP_INTEGRATION.md) 참조.
...
```
mcp__korean-law__search_law {"query": "[검색어]"}
```

여러 결과 반환(lawId/MST 포함), 사용자가 어느 것인지 선택 필요.

### 패턴 2: 조문 가져오기 (위임조문 체인 포함)

```
mcp__korean-law__get_law_text {"query": "개인정보보호법 제28조의8"}
mcp__korean-law__legal_research {"task": "law_system", "query": "개인정보보호법 제28조의8"}
```

`legal_research`의 `law_system` task로 본법-시행령-시행규칙 위임 관계를 추적.

### 패턴 3: 판례 검색

```
mcp__korean-law__search_decisions {"query": "PIPA 영향평가 의무", "domain": "판례"}
```
...
응답으로 조문 실존·현행 여부 확인. 인용 판례는 `{"mode": "cite_check"}`로 폐기·변경 확인.
...
| Available integrations | 연결 가능한 통합 | **국가법령정보 MCP 추가**, 다른 통합 KR화 |
...
| Handling retrieved results | 가져온 결과 처리 | 출처 태그를 KR로 (`[국가법령정보]` 등) |
...
```markdown
> **침묵 보완 금지.** 국가법령정보 검색이 빈 결과면 보고 후 멈춤. 옵션 제시:
> (1) 쿼리 확장, (2) 다른 도구, (3) 웹 검색(`[웹 검색 — 검증 필요]` 태그), (4) 미검증 플래그 후 멈춤.
>
> **출처 attribution 태그:**
> - `[국가법령정보]` — 국가법령정보 MCP 결과
> - `[법제처]` — 법령정보센터 직접
> - `[국회 의안시스템]` — 의안 검색 결과
...
| 영역 | 핵심 |
|---|---|
| 입법 모니터링 | 국회 의안정보시스템 (국가법령정보 커넥터 범위 밖 — 직접 검색), 법제처 입법예고 |
| 행정규칙 | 국가법령정보 행정규칙 데이터 |
